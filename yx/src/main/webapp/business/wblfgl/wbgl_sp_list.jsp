...
<script type="text/javascript">
            ...
            colModel:[
                ...
                {name:'id',index:'', fixed:true, sortable:false, resize:true,
                    formatter:function(cellvalue, options, rowObject){
                        var zt = rowObject.zt;
                        if(zt == '待审批' || zt == '退回'){
                            return "<button class='btn btn-info btn-mini' onclick='spWbgl("+cellvalue+")' title='测试' ><i class='ace-icon fa fa-pencil '>审核</i></button>";

                        }else{
                            return "<button class='btn btn-info btn-mini' onclick='showWbgl(\"+cellvalue+\")' title='测试' ><i class='ace-icon fa fa-eye '>查看</i></button>" +
                                "<button class='btn btn-danger btn-mini' onclick='gdWbgl("+cellvalue+")' title='测试' ><i class='ace-icon fa fa-check '>归档</i></button>";
                        }
                    }
                },
            ]
    ...
    //审批外宾接待申请
    function spWbgl(userId){
        layer.newpage({
            area: ['900px', ($(window).height()-20)+"px"],
            title:'外宾接待申请【审批】',
            content:'business/wblfgl/wbgl_sp.jsp',
        });
    }
    function showWbgl(userId){
        layer.newpage({
            area: ['900px', ($(window).height()-20)+"px"],
            title:'查看接待情况',
            content:'business/wblfgl/wbgl_sp_show.jsp',
        });
    }

    function gdWbgl(userId){
        layer.newpage({
            area: ['500px', "300px"],
            title:'上传归档材料',
